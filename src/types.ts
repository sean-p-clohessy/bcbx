...
export interface ActivityRow { investment_id:string; learner_name:string; staff_ticker:string; business_value:string; amount:number; public_message?:string|null; created_at:string }
export interface BusinessValue { id:string; name:string; description:string; active:boolean; sort_order:number }
export interface Learner { id:string; display_name:string; course_group:string; active:boolean; created_at?:string }
