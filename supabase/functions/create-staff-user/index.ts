...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const body=await request.json(), email=String(body.email??'').trim().toLowerCase(), displayName=String(body.display_name??'').trim(), ticker=String(body.ticker??'').trim().toUpperCase(), role=body.role==='admin'?'admin':'staff', password=String(body.password??'')
